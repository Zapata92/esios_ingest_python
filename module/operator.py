...
class EsiosOperator():
    ...
    def create_description_df(self, esios):
    ...
    def missing_control(self, df, info_col):
        df.replace(to_replace=[None], value=0, inplace=True)
        df_columns = list(df.columns)
        for c in info_col:
            if c not in df_columns:
                print("No se tienen datos del campo {}, se completa con ceros"
                       .format(c))
                df[i] = 0
            else:
                pass
    def calculate_columns(self,df, sum_cols, new_col):
        try:
            df[new_col] = df[sum_cols]
        except:
    ...
    def _etl_sendpostgres_(self, postgres, esios, ind_description,
        ...
        try:
            if self.table == "generacion_tiemporeal":
                ...
                df.drop(
                    columns=["gmedida_oceano_geotermica", "gmedida_biomasa"])
            else:
                pass
        except UnboundLocalError:
            pass
